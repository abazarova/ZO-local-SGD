...
def reg_logreg_function(w, L, X, y):
    r = X.shape[0]
    return logreg_function(w, X, y) + L / r * la.norm(w) ** 2
...
def pseudo_grad_reg_logreg(e, delta, w, params):
    L = params[0]
    X = params[1]
    y = params[2]
    theta = (reg_logreg_function(w + delta * e, L, X, y) - reg_logreg_function(w, L, X, y)) / delta
    return theta * e
...
def reg_logreg_grad(w, params):
    X = params[1]
    y = params[2]
    L = params[0]
    r = X.shape[0]
    return logreg_grad(w, params[1:]) + 2 * L * la.norm(w) / r  
